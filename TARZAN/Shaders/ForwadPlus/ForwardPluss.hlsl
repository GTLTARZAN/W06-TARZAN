...
VS_OUTPUT_SCENE RenderSceneVS(VS_INPUT_SCENE Input)
{
    ...
    Output.PositionWS = mul(Input.Position, (float3x3)World);
...
{
    float4 DiffuseTex = TxDiffuse.Sample(Sampler, Input.TextureUV);
...
float4 RenderScenePS(VS_OUTPUT_SCENE Input) : SV_TARGET
{
    ...
    float3 vNorm = TxNormal.Sample(Sampler, Input.TextureUV).xyz;
    vNorm *= 2;
    vNorm -= float3(1,1,1);
...
#endif
    {
        ...
        if (fLightDistance < fRad)
        {
            ...
            LightColorDiffuse = PointLightBufferColor[nLightIndex].rgb * saturate(dot(vLightDir,vNorm)) * fFalloff;

            float3 vHalfAngle = normalize(vViewDir + vLightDir);
            LightColorSpecular = PointLightBufferColor[nLightIndex].rgb * pow(saturate(dot(vHalfAngle, vNorm)), 8) * fFalloff;
        }
...
#endif
    {
        ...
        if (fLightDistance < fRad && fCosineOfCurrentConeAngle > fCosineOfConeAngle)
        {
            ...
            // k=20: -(1/20)*(1 - 21/(1+20*x^2))
            float fFalloff = -0.05 + 1.05 / (1 + 20 * x * x);
            LightColorDiffuse = SpotLightBufferColor[nLightIndex].rgb * saturate(dot(vToLightNormalized,vNorm)) * fFalloff * fRadialAttenuation;

            float3 vHalfAngle = normalize(vViewDir + vToLightNormalized);
            LightColorSpecular = SpotLightBufferColor[nLightIndex].rgb * pow(saturate(dot(vHalfAngle, vNorm)), 8) * fFalloff * fRadialAttenuation;
        }
